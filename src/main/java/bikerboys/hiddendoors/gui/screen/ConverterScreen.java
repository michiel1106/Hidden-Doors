...
public class ConverterScreen extends HandledScreen<ConverterScreenHandler> {
    ...
    private void renderRecipeBackground(DrawContext context, int mouseX, int mouseY, int x, int y, int scrollOffset) {
        for(int i = this.scrollOffset; i < scrollOffset && i < ((ConverterScreenHandler)this.handler).getAvailableRecipeCount(); ++i) {
            ...
            if (i == ((ConverterScreenHandler)this.handler).getSelectedRecipe()) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (this.canCraft) {
            ...
            for(int l = this.scrollOffset; l < k; ++l) {
                ...
                if (d >= 0.0 && e >= 0.0 && d < 16.0 && e < 18.0 && ((ConverterScreenHandler)this.handler).onButtonClick(this.client.player, l)) {
                    MinecraftClient.getInstance().getSoundManager().play(PositionedSoundInstance.master(SoundEvents.UI_STONECUTTER_SELECT_RECIPE, 1.0F));
                    this.client.interactionManager.clickButton(((ConverterScreenHandler)this.handler).syncId, l);
                    return true;
                }
            }
    ...
    private boolean shouldScroll() {
        return this.canCraft && ((ConverterScreenHandler)this.handler).getAvailableRecipeCount() > 12;
    }

    protected int getMaxScroll() {
        return (((ConverterScreenHandler)this.handler).getAvailableRecipeCount() + 4 - 1) / 4 - 3;
    }

    private void onInventoryChange() {
        this.canCraft = ((ConverterScreenHandler)this.handler).canCraft();
        if (!this.canCraft) {
